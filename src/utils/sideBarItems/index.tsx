...
export const usePortfolioManagerSideBarItems =  (currentItem: string) => {
    ...
    const items: navbarRouterItemsProps[] = [
        ...
        {id: 'loan', name: 'Loan', route: `/loan/${currentTabRoute || 'loan-referral'}`, isActive: true, icon: <Icon icon="material-symbols:money-bag-outline" height={"1.2rem"} width={"1.2rem"} color={currentItem === 'Loan' ? '#142854' : '#939cb0'}></Icon>},
...
export const getLoaneeSideBarItems = (currentItem: string, isLoaneeIdentityVerified: boolean) => {
    ...
    const items : navbarRouterItemsProps[] = [
        ...
        {icon: <MdOutlineAccountBalanceWallet color={currentItem === 'Wallet' ? 'meedleBlue' : '#d7d7d7'} className={` h-[1.2rem] w-[1.2rem] ${isLoaneeIdentityVerified ? `${currentItem === 'Wallet' ? 'text-meedlBlue md:text-meedlBlue' : 'text-[#626F8C] md:text-[#626F8C]'} ` : 'text-[#d7d7d7] md:text-[#d7d7d7]'} `}/>, id: 'wallet', name: "Wallet", isActive: true, route: '/wallet'},
        ...
        {icon: <MdOutlineAccountBalanceWallet className={` h-[1.2rem] w-[1.2rem] text-[#d7d7d7] md:text-[#d7d7d7`}  color={currentItem === "Payment" ? '#142854' : '#939CB0'} />, id: 'payment', name: "Payment", isActive: false, route: '/payment'},
    ]
    return items;
}
