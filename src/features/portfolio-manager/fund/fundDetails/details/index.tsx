...
const Details = () => {
  ...
  useEffect(() => { 
    ...
    if (id) { 
      setInvestmentId(id); console.log('InvestmentId set:', id); 
    } 
   }, []); 
  ...
  const detailInfo = [
    ...
    { name: 'Vehicle status', value: <p className='w-14 h-6 bg-success50 flex justify-center items-center rounded-lg'>{data?.data?.status || 'close'}</p> },
    { name: 'Interest rate', value: `${data?.data?.rate || 0}%` },
    { name: 'Total amount in vehicle', value: formatAmount(data?.data?.totalAmountInInvestmentVehicle?.toString() || '0') },
