...
const CohortView = () => {
    ...
    const {
        ...
        ...(["PORTFOLIO_MANAGER", "MEEDL_SUPER_ADMIN", "MEEDL-ADMIN"].includes(user_role || "") && organizationId
            ? {organizationId}
            : {}),
        cohortStatus: cohortTab.toUpperCase(),
        cohortType: ["ORGANIZATION_ADMIN", "ORGANIZATION_SUPER_ADMIN", "ORGANIZATION_ASSOCIATE"].includes(user_role || "") ? "NON_LOAN_BOOK" : organizationTabStatus === "cohort" ? "NON_LOAN_BOOK" : "LOAN_BOOK",
        pageSize: 10,
    ...
    const {
        ...
        cohortName: debouncedSearchTerm, ...(["PORTFOLIO_MANAGER", "MEEDL_SUPER_ADMIN", "MEEDL-ADMIN"].includes(user_role || "") && organizationId
            ? {organizationId}
            : {}),
        programId: programId,
        cohortType: ["ORGANIZATION_ADMIN", "ORGANIZATION_SUPER_ADMIN", "ORGANIZATION_ASSOCIATE"].includes(user_role || "") ? "NON_LOAN_BOOK" : organizationTabStatus === "cohort" ? "NON_LOAN_BOOK" : "LOAN_BOOK",
    ...
    return (
        <div
            className={` ${["PORTFOLIO_MANAGER", "MEEDL_SUPER_ADMIN", "MEEDL_ADMIN", "MEEDL_ASSOCIATE"].includes(user_role || "") ? "md:overflow-scroll max-h-[65vh]" : ""} `}
            style={{
                scrollbarWidth: 'none',
        ...
        >
                        ...
                        {!["PORTFOLIO_MANAGER", "MEEDL_SUPER_ADMIN", "MEEDL_ADMIN", "MEEDL_ASSOCIATE"].includes(user_role || "") ?
                            <div className='z-10'>
                                <DropdownMenu onOpenChange={toggleDropdown}>
                                    ...
                                    <DropdownMenuContent
                                        className='mt-2 relative left-[-30px] md:left-[-100px] px-4 pb-6 z-[1000rem]'>
                                        ...
                                        >
                                            {({isValid, setFieldValue, resetForm}) => (
                                                <Form>
                                                    ...
                                                    >
                                                        <SelectTrigger id='cohortInProgramSelectTrigger'
                                                        ...
                                                        >
                                                            {
                                                                programIsloading ? (<div><SkeletonForLoanOrg/>
                                                                </div>) : listOfPrograms.length === 0 ? (<div>
                                                            ...
                                                            >
                                                                {cohortIsLoading ? (
                                                                    <div id={'loadingLoopIconDiv'}
                                                                        ...
                                                                        <Icon id={'Icon'} icon={loadingLoop} width={24}
                                                                              height={24}/>
                                                                    </div>
                                                                ) : (
                                                                    "Filter"
            ...
            <div
                className={` ${["PORTFOLIO_MANAGER", "MEEDL_SUPER_ADMIN", "MEEDL_ADMIN", "MEEDL_ASSOCIATE"].includes(user_role || "") ? "mt-8" : " mr-auto ml-auto relative w-[96%] mt-12 "}`}>
                <CohortTabs
                ...
                >
                    {["PORTFOLIO_MANAGER", "MEEDL_SUPER_ADMIN", "MEEDL_ADMIN", "MEEDL_ASSOCIATE"].includes(user_role || "") ?
                        <AddCohortInAnOrganization setIsOpen={setIsOpen} organizationId={organizationId}
                                                   cohortRefetch={refetchCohortData}/> :
