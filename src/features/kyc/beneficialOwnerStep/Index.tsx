...
interface IndividualSection extends BaseSection {
    firstName: string;
    lastName: string;
    dob: Date | undefined;
    relationship: string;
...
interface Section extends EntitySection, Partial<Omit<IndividualSection, 'id'>> {}
...
const BeneficialOwnerStep = () => {
    ...
    const [sections, setSections] = useState<Section[]>(entityData.sections || []);
    ...
    const isFormValid = useMemo(() => {
        ...
        if (selectedForm === "entity") {
            const mainFormValid = entityName.trim() !== "" &&
                rcNumber.trim() !== "" &&
            ...
            const sectionsValid = sections.every(section => {
                ...
                if (sectionType === "entity") {
                    return section.entityName?.trim() !== "" &&
                        section.rcNumber?.trim() !== "" &&
                        section.country !== undefined;
                } else if (sectionType === "individual") {
                    return section.firstName?.trim() !== "" &&
                        section.lastName?.trim() !== "" &&
                        section.dob !== undefined &&
                        section.relationship?.trim() !== "" &&
                        section.ownership?.trim() !== "" &&
                        section.proofType?.trim() !== "" &&
                        section.proofFileUrl !== undefined;
                }
                return true;
            });

            return mainFormValid && sectionsValid;
        }
        return true;
    }, [selectedForm, entityName, rcNumber, selectedCountry, sections, sectionTypes]);
    ...
    return (
        <main id="beneficialOwnerStepMain" className={`${inter.className} w-full xl:px-48 grid-cols-1 gap-y-5 grid`}>
            ...
            <section id="beneficialOwnerSection" className={'md:max-w-[30rem] w-full md:mx-auto h-[calc(100vh-250px)] pt-1 overflow-y-auto pr-3'}>
                <Tabs
                    ...
                    <TabsContent id="entityTabContent" value="entity">
                        <main id="entityFormMain" className="grid gap-6">
                            ...
                            {sections.map((section) => (
                                <section key={section.id} className={'relative grid '}>
                                        <Tabs
