...
function CustomMultiselect({
    ...
    const isOptionDisabled = (value: string) => {
        ...
        if (selectedOptions.length > 0) {
            ...
            return isSelectedItemRestricted ? !restrictedItems.includes(value) : value !== selectedOptions[0];
        }
        return false;
    };
    ...
    const toggleSelectAll = (isSelected: boolean) => {
        const newOptions = isSelected ? filterOptions.map((option) => option.value) : [];
        setSelectedOptions(newOptions);
        onValueChange(newOptions);
    };
                        ...
                        {filterOptions.length > 0 ? (
                            filterOptions.map((option) => {
                                ...
                                return (
                                    <div
                                        key={option.value}
                                        className={`flex items-center gap-1 px-2 py-1 hover:bg-[#F4F5F5] cursor-pointer${
