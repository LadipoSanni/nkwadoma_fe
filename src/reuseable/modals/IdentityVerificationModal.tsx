            ...
            <Dialog open={isOpen} onOpenChange={onClose}>
                ...
                <DialogContent className={'max-w-[425px] md:max-w-[533px] [&>button]:hidden gap-6 py-5 px-5'}>
                    <DialogHeader className={'flex py-3'} id="createCohortDialogHeader">
                        <DialogTitle
                            className={`${cabinetGrotesk.className} text-[28px] font-medium text-labelBlue leading-[120%]`}>Provide your details</DialogTitle>
                        <DialogClose asChild>
                        ...
                        <form className={`${inter.className}`} onSubmit={methods.handleSubmit(onSubmit)}>
                            <main className={'grid gap-5'}>
                                <div className={'grid gap-2'}>
                                    ...
                                    />
                                    {methods.formState.errors.bvn && <p className="text-red-500 text-sm">{methods.formState.errors.bvn.message}</p>}
                                    <Collapsible className={'bg-neutral250 rounded grid gap-2 py-2 px-3'} open={isBVNOpen} onOpenChange={setIsBVNOpen}>
                                        <CollapsibleTrigger asChild>
                                            <div className={'flex justify-between cursor-pointer select-none'}>
                                                <p className={'text-black500 text-[14px] leading-[150%] font-normal'}>Why do we need your Bank verification number?</p>
                                                {isBVNOpen ? <MdHorizontalRule id="tuitionBreakdownArrowUp" className={'h-5 w-5 text-primary200'} /> :
                                                    <MdOutlineAdd id="tuitionBreakdownArrowDown" className={'h-5 w-5 text-primary200'} />}
                                            </div>
                                        </CollapsibleTrigger>
                                        <CollapsibleContent>
                                            <p className="text-black400 text-[14px] leading-[150%] font-normal">We request for your BVN to verify your identity and confirm that the account you registered with is yours. It is also a KYC requirement for all financial institutions by the Central Bank of Nigeria (CBN).</p>
                                        </CollapsibleContent>
                                    </Collapsible>
                                </div>
                                <div className={'grid gap-2'}>
                                    ...
                                    <Input
                                        ...
                                        className={'p-4 focus-visible:outline-0 shadow-none focus-visible:ring-transparent rounded-md h-[3.375rem] font-normal leading-[21px] text-[14px] placeholder:text-grey250 text-black500 border border-solid border-neutral650 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none'}
                                    />
                                    {methods.formState.errors.nin && <p className="text-red-500 text-sm">{methods.formState.errors.nin.message}</p>}
                                    <Collapsible className={'bg-neutral250 rounded grid gap-2 py-2 px-3'} open={isNINOpen} onOpenChange={setIsNINOpen}>
                                        <CollapsibleTrigger asChild>
                                            <div className={'flex justify-between cursor-pointer select-none'}>
                                                <p className={'text-black500 text-[14px] leading-[150%] font-normal'}>Why do we need your National identification number?</p>
                                                {isNINOpen ? <MdHorizontalRule id="tuitionBreakdownArrowUp" className={'h-5 w-5 text-primary200'} /> :
                                                    <MdOutlineAdd id="tuitionBreakdownArrowDown" className={'h-5 w-5 text-primary200'} />}
                                            </div>
                                        </CollapsibleTrigger>
                                        <CollapsibleContent>
                                            <p className="text-black400 text-[14px] leading-[150%] font-normal">We request for your NIN to verify your identity and confirm that the account you registered with is yours. It is also a KYC requirement for all financial institutions by the Central Bank of Nigeria (CBN).</p>
                                        </CollapsibleContent>
                                    </Collapsible>
                                </div>
            ...
            <Dialog open={isSecondModalOpen} onOpenChange={setIsSecondModalOpen}>
                ...
                <DialogContent className={'max-w-[425px] md:max-w-[460px] [&>button]:hidden gap-6 py-5 px-5'}>
                    <DialogHeader className={'flex py-3'} id="secondModalHeader">
                        <DialogTitle
                            className={`${cabinetGrotesk.className} text-[28px] font-medium text-labelBlue leading-[120%]`}>Liveness check</DialogTitle>
                    </DialogHeader>
                        ...
                        {showCamera ? (
                            ...
                            <>
                                ...
                                <div className={'grid gap-4'}>
                                    <h1 className={'text-black500 text-[16px] leading-[150%] font-medium'}>Allow camera access</h1>
                                    <div className={'grid gap-3'}>
                                        <p className={'text-black400 text-[14px] leading-[150%]'}>To continue the verification process, we need access to your device’s camera</p>
                                        <p className={'text-black400 text-[14px] leading-[150%] self-stretch'}>Give your browser access a permission to user your camera by clicking the “Allow” button in the window in upper left corner or user your mobile device to pass the verification</p>
                                    </div>
