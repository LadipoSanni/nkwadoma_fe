...
interface ItemListProps {
    items: { itemName: string, amount: string,currency: string }[];
    setItems: (items: { itemName: string, amount: string,currency: string }[]) => void;
    handleDeleteItem: (index: number) => void;
}

const ItemList: React.FC<ItemListProps> = ({ items, setItems, handleDeleteItem }) => {

    const [selectCurrency] = useState('NGN');
                ...
                <div className="grid gap-2">
                    <div className="flex gap-2">
                        ...
                        <div className={'flex justify-between gap-2 items-center'}>
                            <Input type="number" id={`itemAmount${index}`} name={`itemAmount-${index}`} placeholder="0.00" className="p-4 focus-visible:outline-0 w-[6.25rem] md:w-[8.25rem] shadow-none focus-visible:ring-transparent rounded-md h-[3.10rem] font-normal leading-[21px] text-[14px] placeholder:text-grey150 text-black500 border border-solid border-neutral650 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" value={item.amount} onChange={(e) => {
                                const newItems = [...items];
