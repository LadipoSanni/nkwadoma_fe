...
describe("EmptyState component", () => {
    it('should test that EmptyState component does not exist', () => {
    ...
    it('should est that EmptyState component exist', () => {
        const {queryByTestId} = render(<EmptyState title={`Loan Disbursal`}/>);
    ...
    it('should test that empty state component renders the correct number of blocks', () => {
        const {getAllByTestId} = render(<EmptyState title="Loan books will show here"/>);
        const blocks = getAllByTestId(/empty_StateBlock/i);
        expect(blocks.length).toBe(3);
    });

    it("should test that box shadow is applied to second block only", () => {
        const {getByTestId} = render(<EmptyState title="Loan refferers will show here"/>);
