    ...
    <div className={`px-4 md:mt-36 mt-28 ${className}`}>
       ...
       <div>
        <div className='flex justify-center text-center' >
         ...
         <p id='bodyMessageId' className='text-foundationBlue800 md:w-96 w-72 '>There are no {optionalFilterName} {name}s available yet. { !condition?<span> To create a  <span className='lowercase'>{name}</span>, click on the  <span className='font-semibold lowercase'>create {name}</span>  </span>: ""}</p>
          </div>
       </div>
    </div>
  )
