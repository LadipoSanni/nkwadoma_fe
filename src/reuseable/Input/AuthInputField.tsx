...
const AuthInputField: React.FC<ReusableInputProps> = ({label, id, endAdornment, type, errorMessage, ...props}) => {
    ...
    const renderEndAdornment = () => {
        if (typeof endAdornment === 'string') {
            ...
            return (
                <span id={`endAdornment${id}`} style={{cursor: 'pointer'}} onClick={handleToggleVisibility}>
                    {isPasswordVisible ?
    ...
    return (
        <div id={`customInputField-${id}`} className={`${inter.className} flex flex-col gap-[8px]`}>
            ...
            {errorMessage && <div id={`${id}ErrorMessageBlock`} className={'flex px-4 items-center'}><p id={`errorMessage${id}`} className="text-error600 text-[14px] font-normal">{errorMessage}</p>
            </div>}
        </div>
    );
};
