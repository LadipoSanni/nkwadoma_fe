...
import { insertSpaceCapitalized, toSentenceCase } from "@/utils/GlobalMethods";
...
const InvestmentCard: React.FC<InvestmentCardProps> = ({
  ...
  return (
    ...
    >
      <div
        id='investment-type-segment'
        data-testid={`investment-type-segment`}
      ...
      >
        <div
          id={`type`}
          data-testid={`type`}
          className={`py-5 px-4 flex flex-col`}
        >
          <div
            id='investmentTypeId'
          ...
          >
            {investmentVehicleType
              ? investmentVehicleType.charAt(0).toUpperCase() +
                investmentVehicleType.slice(1).toLowerCase()
              : ""}
          </div>
        </div>
      ...
      <div id={"detailsSectionId"} className={`pt-5`}>
        <div
          id={"minidetailsId"}
          className={`flex bg-[#F6F6F6] items-center gap-2 rounded-lg px-2 py-1 w-fit ${
            status === statusValue ? "py-[11px]" : ""
          }`}
        >
          <span
            id={"fundrasingId"}
          ...
          <div
            id={"statusDivId"}
            className={`bg-meedlWhite p-1 border rounded-lg ${borderClass} ${
              status === statusValue ? "hidden" : ""
            }`}
          >
            <span
              id={"statusId"}
        ...
        <div
          id={"investmentVehicleNameId"}
          className={`${cabinetGrotesk.className} pt-3 font-medium text-xl text-[#212221] `}
        >
          {toSentenceCase(investmentVehicleName)}
        </div>
        <div
          id={"percentageId"}
          className={`${inter.className} text-[#4D4E4D] font-normal text-sm`}
        >
          {percentage}% interest
        </div>

        <div
          id={"HandleCardDetailsId"}
          className={`${inter.className} py-3 font-normal text-sm text-meedlBlue underline cursor-pointer`}
