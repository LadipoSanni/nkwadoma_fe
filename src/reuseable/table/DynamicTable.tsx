...
const DynamicTable = <T extends { [key: string]: string }>({
    ...
    return (
        <div data-testid="dynamicTableContainer" className={`w-[100%]`}>
            <Paper data-testid="dynamicTablePaper" sx={{width: '100%', overflow: 'hidden'}} elevation={0}>
                <div data-testid="dynamicTableBorder"
                     className='border-[1px] border-learnSpaceGray border-solid rounded'>
                    <TableContainer data-testid="dynamicTableScrollbar" className='scrollbar-custom'
                                    sx={{height: `${tableHeight}vh`}}>
                        <Table data-testid="dynamicTable" stickyHeader
                               sx={{width: '100%', border: '#d0d5dd', height: 'auto', backgroundColor: '#ffffff'}}>
                            <TableHead data-testid="dynamicTableHead" sx={{backgroundColor: '#e7e7e7'}}>
                                <TableRow data-testid="dynamicTableHeadRow"
                                          style={{position: "sticky", top: 0, backgroundColor: "#fafbfc", zIndex: 1}}>
                                    {tableHeader.map((column) => (
                                        <TableCell data-testid={`dynamicTableHeadCell${column.id}`}
                                                   sx={{backgroundColor: '#fafbfc'}} key={column.id}>
                                            <div data-testid={`dynamicTableHeadCellDiv${column.id}`}
                            ...
                            <TableBody data-testid="dynamicTableBody">
                                {paginatedData.map((row, rowIndex) => (
                                    ...
                                    >
                                        {tableHeader.map((column) => (
                                            <TableCell data-testid={`dynamicTableBodyCell${rowIndex}${column.id}`}
                                                       key={column.id}>
                                                <div data-testid={`dynamicTableBodyCellDiv${rowIndex}${column.id}`}
