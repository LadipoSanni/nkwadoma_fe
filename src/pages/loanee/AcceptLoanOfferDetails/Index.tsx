                            ...
                            {currentTab === 2 && (
                                <section>
                                    ...
                                    <div className="flex items-start gap-4 bg-grey105 p-5">
                                        ...
                                        >
                                            I have read, understood and I agree with the <span
                                            className={' cursor-pointer text-meedlBlue underline'}>Loan terms & conditions </span>
                                        </label>
                                    </div>
                                </section>
                            )}
                        ...
                        {currentTab === loanRequestDetailsTab.length - 1 ? (
                            <DropdownMenu onOpenChange={(open) => setIsDropdownOpen(open)}>
                                <DropdownMenuTrigger asChild>
                                    <Button
                                        className={`flex gap-2 w-full md:w-[10.875rem] h-[3.5625rem] ${isCheckboxChecked ? 'bg-meedlBlue hover:bg-meedlBlue' : 'bg-blue100 hover:bg-blue100'}`}
                                        disabled={!isCheckboxChecked}
                                    >
                                        Make a decision
                                        <span>{isDropdownOpen && isCheckboxChecked ?
                                            <MdKeyboardArrowUp className={'h-6 w-6 text-meedlWhite'}/> :
                                            <MdKeyboardArrowDown className={'h-6 w-6 text-meedlWhite'}/>}</span>
                                    </Button>
                                </DropdownMenuTrigger>
                                {
                                    isCheckboxChecked && (
                                        <DropdownMenuContent className={'w-[176px] h-[86px] p-1 grid gap-1 '}>
                                            <DropdownMenuItem
                                                ...
                                                onSelect={() => console.log('Option 1 selected')}>Accept loan
                                            ...
                                            <DropdownMenuItem
                                                ...
                                                onSelect={() => setIsModalOpen(true)}>Decline loan offer</DropdownMenuItem>
                                        </DropdownMenuContent>
                                    )
                                }
                            </DropdownMenu>
