...
// import ChangeOrganizationInfinityScroll from "./Change-organization-infinity-scroll";
...
//     const {data, isLoading,isFetching} = useViewOrganizationsQuery(element);
//     const {
//         data: searchResults, isLoading: isSearchLoading, isFetching: isSearchFetching
//     } = useSearchOrganisationByNameQuery(searchElement, {skip: !debouncedSearchTerm});
//     const organisationList: ChangeOrganization[] = debouncedSearchTerm?.length > 0 ? searchResults?.data.body || [] : data?.data?.body
...
//                             </div> :
//                             (<div
//                                 className={`${styles.organizations} space-y-3 lg:max-h-[56.5vh] max-h-[50vh] overflow-y-auto  py-2 grid md:grid`}
                                
//                                 >
//                                 {debouncedSearchTerm ? (
//                                        !isTyping  && searchResults?.data?.body?.length === 0?
//                                         <div className="w-full relative bottom-20 px-16 md:px-0 ">
...
const ChangeInstitutionModal = () => {
  ...
  useEffect(() => {
    if (isSearching && searchResults?.data?.body) {
      if (page === 0) {
        setOrganizations(searchResults.data.body);
      } else {
        const newOrganizations = searchResults.data.body.filter(
  ...
  const getLoanCounts = (org: ChangeOrganization) => {
    switch (currentTab) {
      case 'Loan requests':
        return roundUpAmount(org?.loanRequestCount?.toString());
      case 'Loan offers':
        return roundUpAmount(org?.loanOfferCount?.toString());
      case 'Disbursed loan':
        return roundUpAmount(org?.loanDisbursalCount?.toString());
      case 'Loan referrals':
        return roundUpAmount(org?.loanReferralCount?.toString());
      default:
        return '0';
    }
  };

  const roundUpAmount = (number: string | undefined) => {
    if (!number) return '0';
    const numberGotten = Number(number);
    if (numberGotten >= 1000 && numberGotten < 10000) {
      return number.charAt(0) + '.' + number.charAt(1) + 'k';
  ...
  return (
    <div>
        ...
        {showLoading ? (
          ...
          >
            <InfiniteScroll
              ...
              next={fetchMoreData}
              hasMore={hasMore}
              loader={organizations.length > 0 && hasMore ? <SkeletonForLoanOrg /> : null}
              endMessage={
                organizations.length > 0 && (
                  <div style={{ textAlign: 'center', padding: '1rem' }}>
                    <p className="font-medium">No more organizations to load</p>
                  </div>
                )
              }
            ...
            >
              {showSearchEmptyState ? (
                <div className="w-full relative bottom-20 px-16 md:px-0">
                  <SearchEmptyState name={"organization"} icon={MdSearch} />
                </div>
              ) : showEmptyState ? (
                <div className='w-full h-fit md:w-full md:h-full'>
                ...
                organizations.map((organization: ChangeOrganization, index: number) => {
                  const initial = getInitials(organization.name);
                  return (
                    <button
                    ...
                    >
                      ...
                      <div className='flex md:flex bg-grey105 md:bg-grey105 px-2 py-1 md:px-3 md:py-2 rounded-full md:rounded-full w-fit h-fit md:w-fit md:gap-2 md:h-fit md:place-items-between place-items-center gap-3'>
                        <span className='text-xs md:text-xs md:break-normal w-fit md:w-fit'>{currentTab}</span>
                        <div className='w-fit h-fit ring-1 ring-[#E1EEFF] rounded-md px-1 py-1 md:w-fit md:h-fit md:ring-1 md:ring-[#E1EEFF] md:rounded-md md:px-1 md:py-1'>
                          <div className='bg-[#E1EEFF] h-fit flex place-content-center w-fit'>
                            <span className='text-xs mt-auto mb-auto text-meedlBlue'>{getLoanCounts(organization)}</span>
                          </div>
                        </div>
                      </div>
                    </button>
      ...
      <div className="py-4 md:flex md:justify-end h-fit grid gap-3 md:gap-4 md:h-fit w-full md:w-full">
        <Dialog.Close asChild>
          <Button
            onClick={handleCancel}
            variant={"outline"}
