...
function ViewAllRequestedFinancier() {
   ...
   const [tabStates, setTabStates] = useState<Record<string, TabState>>({
        pending: { pageNumber: 0, totalPages: 0, hasNextPage: false },
        declined: { pageNumber: 0, totalPages: 0, hasNextPage: false },
    });
    ...
    useEffect(()=> {
        const currentData = debouncedSearchTerm ? searchResults?.data : data?.data;
  ...
  const handleSearchChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const tableHeader = [
        ...
        { 
            ...
            sortable: true, 
            id: "requestedBy", 
            selector: (row: TableRowData ) => capitalizeFirstLetters(row?.invitedBy?.toString())
        },
        { 
        ...
        { 
            ...
            sortable: true, 
            id: "invitedDate", 
            selector: (row: TableRowData ) =>  row?.requestedInvitationDate ? formatMonthInDate(String(row?.requestedInvitationDate)) : formatMonthInDate(new Date().toISOString())
        }
    ]
                    ...
                    <TabsContent value="declined">
                        <div className='mt-4' data-testid="table">
                            <Table 
                                ...
                                isLoading={isLoading || isFetching || isSearchfetching || isSearchloading }
                                hasNextPage={currentTabState.hasNextPage}
                                pageNumber={currentTabState.pageNumber} // Use single pageNumber
                                setPageNumber={handlePageChange}
                                totalPages={currentTabState.totalPages}
                                sx={userRole !== "MEEDL_ADMIN"?'cursor-pointer' : ""}
                                condition={true}
