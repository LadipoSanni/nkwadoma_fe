...
function  OrganizationDetailLayout({children}:props) {
  ...
  return (
    <div>
        ...
        <div className={`relative  ${["MEEDL_SUPER_ADMIN"].includes(user_role || "") && !["INVITED","PENDING_APPROVAL"].includes(organizationTabStatus)?  "bottom-10" : "bottom-0"}`}>
          {[ "MEEDL_SUPER_ADMIN"].includes(user_role || "") &&  !["INVITED","PENDING_APPROVAL"].includes(organizationTabStatus)  &&  <div className='flex justify-end px-6 relative '>
