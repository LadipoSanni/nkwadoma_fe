...
describe("LoanEmptyState component", () => {
    ...
    // beforeEach(() => {
    //     render(
    //         <LoanEmptyState title={`Loan Disbursal`} />
    //     )
    // })
    ...
    it('should est that LoanEmptyState component exist', () => {
        const {queryByTestId} = render(<LoanEmptyState description={''} icon={''} iconBg={'r'} id={'testing1'} title={`Loan Disbursal`}/>);

        expect(queryByTestId('testing1')).toBeInTheDocument();
    });
    ...
    it('should extract and display the first and second word of the title to lowercase in the description', () => {
        ...
        render(<LoanEmptyState title={title} descriptionId={'emptyStateDescription'}  description={expectedDescription} icon={''} iconBg={'r'} id={'testing2'}/>);
    ...
    //     const {getAllByTestId} = render(<LoanEmptyState  description={''} icon={''} iconBg={'r'} id={'testing1'} title="Loan books will show here"/>);
    //     const blocks = getAllByTestId(/empty_StateBlock/i);
    //     expect(blocks.length).toBe(3);
    // });

    // it("should test that box shadow is applied to second block only", () => {
    //     const {getByTestId} = render(<LoanEmptyState  description={''} icon={''} iconBg={'r'} id={'testing1'} title="Loan refferers will show here"/>);
    ...
    it('should test that empty state component renders the description with the first two words of the title in it', () => {
        ...
        render(<LoanEmptyState  description={''} icon={''} iconBg={'r'} id={'testing1'} title={title}/>);
